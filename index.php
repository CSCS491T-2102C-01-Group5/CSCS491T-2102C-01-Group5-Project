...
<html>
  ...
  <body>
    ...
    <div id="scProduct"><?php
      ...
      <div class="pdt">
        <img class="pImg" src="public/<?= $row['book_coverImg'] ?>"/>
        <h3 class="pName"><?= $row['book_title'] ?></h3>
        <div class="pPrice">$<?= $row['book_price'] ?></div>
        <div class="pDesc"><?= $row['book_description'] ?></div>
        <input class="pAdd bRed" type="button" value="Add to cart" onclick="cart.add(<?= $row['book_id'] ?>);"/>
      </div>
